...
const DEFAULT_IMAGE_SIZE = { width: 400, height: 600 } as const;
...
async function loadState(): Promise<ScriptState> {
  try {
...
async function saveState(state: ScriptState): Promise<void> {
...
function storagePathForCharacter(characterId: string): string {
  return `${STORAGE_FOLDER}/${characterId}.webp`;
}

function asPublicUrl(supabaseUrl: string, bucketName: string, storagePath: string): string {
  const normalizedBase = supabaseUrl.replace(/\/$/, "");
  return `${normalizedBase}/storage/v1/object/public/${bucketName}/${storagePath}`;
}

function isPublicBucketUrl(url: string, bucketName: string): boolean {
  return url.includes(`/storage/v1/object/public/${bucketName}/`);
}

function extractBucketPath(url: string, bucketName: string): string | null {
...
function candidateQueries(character: CharacterRow): string[] {
  const aliases = Array.isArray(character.aliases) ? character.aliases : [];
  return unique([character.name, ...aliases]);
}

function scoreCandidate(query: string, candidate: JikanCharacter): number {
...
function pickBestCandidate(query: string, candidates: JikanCharacter[]): JikanCharacter | null {
  let best: JikanCharacter | null = null;
  let bestScore = 0;
...
async function searchImageForCharacter(character: CharacterRow): Promise<{
  query: string;
  sourceImageUrl: string;
  matchedName: string;
} | null> {
  ...
  for (const query of queries) {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        const response = await axios.get<JikanResponse>(`${JIKAN_BASE_URL}/characters`, {
...
async function fetchActiveCharacters(
  supabaseClient: ReturnType<typeof createClient<Database>>,
  limit: number | null,
...
async function updateCharacterImageUrl(
  supabaseClient: ReturnType<typeof createClient<Database>>,
  character: CharacterRow,
  imageUrl: string,
): Promise<void> {
...
function summarizeState(state: ScriptState): string {
  const counts = {
    completed: 0,
    skipped: 0,
    failed: 0,
